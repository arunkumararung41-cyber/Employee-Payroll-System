...
abstract class Employee{   //abstraction class
    private String name;
    private int id;
    public Employee(String name,int id){ //paramatrise constructor 
        this.name=name;
        this.id=id;
    }
    public String GetName(){   // getter methord     // we are not giveing direct access of data
        return name;
    }
    public int GetId(){   // getter methord
        return id;
    }
    abstract double CalculateSalary();   //abstract methord 
    @Override
...
class FullTimeEmployee extends Employee{  // showing error if we didnot impliment abstract class methord
...
class PayRollSystem{
    private ArrayList<Employee> employeeList;   // initialisation of arraylist   Employee data type
...
public class main {
    public static void main(String[] args) {
        ...
        FullTimeEmployee emp1 = new FullTimeEmployee("akshit",1 , 90000);
        PartTimeEmployee emp2 = new PartTimeEmployee("harsh", 2, 50, 2000);
        ...
        System.out.println("Remaining employee detail are");
        payrollsystem.DisplayEmployees();
    }
